  ...
  return (
    ...
    >
      {Icon && (
        <div
          className={cn(
            'summary-card__icon flex h-8.5 w-8.5 sm:h-10 sm:w-10 shrink-0 items-center justify-center rounded-xl mt-0.5 transition-colors',
      ...
      <div className="summary-card__content min-w-0 flex-1">
        ...
        <div className="summary-card__value-row mt-1 flex flex-wrap items-center gap-1.5 sm:gap-2">
          <span className="summary-card__value text-xl sm:text-2xl font-black text-slate-900 dark:text-white whitespace-nowrap">
            {value ?? '—'}
          </span>
          {badge && <AppBadge variant={badgeVariant} size="sm" className="shrink-0 text-[10px]">{badge}</AppBadge>}
        </div>
        {description && (
